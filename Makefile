...
DOCKERFILE_PROD  = hack/docker/my-operator/in.Dockerfile
DOCKERFILE_DBG   = hack/docker/my-operator/dbg.Dockerfile
...
install:
	@cd ../installer; \
	APPSCODE_ENV=dev KUBEDB_OPERATOR_TAG=$(TAG) KUBEDB_CATALOG=mysql ./deploy/kubedb.sh --operator-name=$(BIN) --docker-registry=$(REGISTRY) --image-pull-secret=$(REGISTRY_SECRET)
...
stash-install:
	...
	@curl -fsSL https://github.com/stashed/catalog/raw/master/deploy/script.sh | bash -s -- --catalog=stash-mysql --docker-registry=stashed

.PHONY: stash-uninstall
stash-uninstall:
	@curl -fsSL https://github.com/stashed/catalog/raw/v0.9.0-rc.2/deploy/script.sh | bash -s -- --catalog=stash-mysql --uninstall || true
